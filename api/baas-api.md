...
本文档介绍了如何快速调用公信宝BaaS 存储API，适用于API开发者，需要一定的编程基础。因主链存在存储空间大小的限制，我们的解决方法是将文件本体存储在侧链（IPFS）上，将侧链产生的文件ID C-ID返还至主链进行存证。在主链上保存的C-ID、MD5和时间戳可以保证上链数据不可篡改的基本原则。具体BaaS存储流程见下图
